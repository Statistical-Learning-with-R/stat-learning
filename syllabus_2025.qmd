...
| Day                | Time                                                                               |
|--------------------|------------------------------------------------------------------------------------|
| Tuesdays , Thursdays          | 3:10pm - 5pm, in-person (25-106)                                     |
...
not_here_dates <- c(
  ...
  #ymd("2024-11-11"),
  # Tgiving Break
  seq(ymd("2025-11-24"), ymd("2025-11-28"), by = 1)
  )

remote_dates <- c(
)

exam_dates <- c(ymd("2025-10-29"), 
                ymd("2025-12-03")
                )

project_dates <- c(ymd("2025-10-27"), 
                ymd("2025-12-01"),
                #ymd("2024-12-08"),
                ymd("2025-12-08")
                )
...
finals_week <- seq(ymd("2025-12-08"), ymd("2025-12-13"), by = 1)
...
Cal <- tibble(date = seq(ymd("2025-09-01"), ymd("2025-12-31"), by=1))  %>%
...
class_cal <- Cal %>% 
  ...
  scale_fill_manual(values=c("Class Day"="purple", 
                  ...
                  "Project Checkpoint" = "turquoise"),
                    ...
                    breaks=c("Finals Week", "Holiday", "Class Day", "Remote Class", "In-Class Exam", "Project Worktime"))
class_cal
...
# 
# project_days <- filter(Cal, category == "Project Worktime") %>%
#   mutate(topic = c("Project Plan",
#                    "Project Draft", 
#                    "Project Poster Session"),
#           time = c("11:59pm", "11:59pm", "10:00am")) %>%
#    mutate(Date = format(date, "%b %e"),
#           sem_week = week(date))

class_days <- tibble(
  Week = paste("Week", 0:11),
  Topic = c(
    "Setup and Pre-test",
    "Intro to tidymodels; Review of linear models",
    ...
    "Neural Networks",
    "Project, Exam 2",
    "Finals Week - Poster Presentations"
    ))
```
...
In the usual way, you have a longer homework assignment each week. However, this homework is completed in two phases:

#### Phase I: First attempt

You must make a first pass at the homework *on your own*.  During this phase, you may **not use generative AI tools in any way**, and if you do discuss with other students, you must still ensure that all your work is individual and not identical. 

Phase I is due **Sundays at 11:59pm** to **Gradescope**.  It is graded only on *completeness*.

#### Phase II: Final submission

In class on Mondays, you will be randomly placed into a team, and given 20 minutes of class time to discuss the assignment and ask me questions.  Then, you must come up with a polished, fully correct submission.  

For this phase, you *may* use genAI to help, if you properly cite it (see below).  However, you still may *not* copy-paste answers from genAI; this is plagiarism.

Phase II is due **Tuesdays at 11:59pm** to **Gradescope**.  It is graded only on *correctness*.

#### Why Two-Phase Homework?

In the era of generative AI, we are often seeing students lacking the experience of, as we call it in education, "productive struggle". 

I don't like the word "struggle" - I never want you to feel frustrated to the point of despair - but I do believe that important skills and mental muscles develop in that moment where you sit down with a new problem and try to figure out where to begin.  I also believe there is a lot of value in trying to look at an answer (yours or someone else's) and determine if it's correct or not.

The goal of the Phase I homework and subsequent in-class time is to lower the grading stakes and give you the breathing room to feel comfortable trying the homework without genAI. I am the only one who can see your Phase I submission, so you can turn in something completely and totally wrong and still get full credit, as long as it's clear you tried everything!  

Then, in Phase II, the homework mimics more of a "real-world" setting: you can use all the tools at your disposal (your team, genAI, etc.) to accomplish the goals, as long as you do not plagiarize.
...
* **Phase 2:** Executive summary - a brief, action-oriented overview of your results and findings.
