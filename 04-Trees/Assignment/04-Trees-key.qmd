...
# drop veil type because it's pointless
...
mush_wflow <- workflow() %>%
  add_model(tree_spec) %>%
  add_recipe(recipe(class ~ ., mushrooms))
...
# There are 22 variables. Let's try up to 10 allowed per tree.
mtry_grid <- grid_regular(mtry(c(1, 10)), levels = 5)
...
The output is a bit useless!

In fact, we get a warning telling us that some probabilities were 0 or 1.  This is saying that we can't estimate "log-odds" of an event when we've already found a category (for example, brown-spored mushrooms that smell like almonds) that is 100% pure.
